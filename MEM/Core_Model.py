    ...
    for tech_dic in tech_list:
        ...
        # Assumed to be unidirectional for simplicity !!!
            ...
            if 'efficiency' in tech_dic:
                ...
                efficiency = ((1 - tech_dic['converter_pair_loss'] / 2) * 
                              (1 - (tech_dic['line_loss'] * tech_dic['length'])) * 
                              (1 - tech_dic['converter_pair_loss'] / 2))
                if verbose:
                    print('Warning: No efficiency specified for ', tech_name,
                          '. We assume an initial efficiency of 1, a converter loss of ',
                          tech_dic['converter_pair_loss'] / 2 * 100, '% a line loss of ',
                          tech_dic['line_loss'] * 100 * 1000,'% per 1000 km, and a second converter loss of ',
                          tech_dic['converter_pair_loss'] / 2 * 100, '%', sep = '')
            ...
            if 'fixed_co2' in tech_dic:
                fnc2min += capacity * tech_dic['line_cost_land'] * tech_dic['fraction_land'] * tech_dic['length'] * delta_t * num_time_periods
                fnc2min += capacity * tech_dic['line_cost_sea'] * (1 - tech_dic['fraction_land']) * tech_dic['length'] * delta_t * num_time_periods
                fnc2min += capacity * tech_dic['converter_pair_cost'] * delta_t * num_time_periods
                fnc2min += capacity * tech_dic['fixed_co2'] * delta_t * case_dic['co2_price'] * num_time_periods
            else:
                fnc2min += capacity * tech_dic['line_cost_land'] * tech_dic['fraction_land'] * tech_dic['length'] * delta_t * num_time_periods
                fnc2min += capacity * tech_dic['line_cost_sea'] * (1 - tech_dic['fraction_land']) * tech_dic['length'] * delta_t * num_time_periods
                fnc2min += capacity * tech_dic['converter_pair_cost'] * delta_t * num_time_periods
