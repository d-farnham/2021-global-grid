extract_trans_dispatch_by_wire_and_timestep = function(year = 2018,
...
# only include the outputs of the wires
trans_dispatch_gg = trans_dispatch_gg0 %>% dplyr::filter(IN == FALSE) %>%
                                           dplyr::mutate(dispatch_out = dispatch) %>%
                                           dplyr::select(-IN, -dispatch)
...
trans_dispatch = trans_dispatch_gg %>% merge(time_transform,
                                             by = 'time_index') %>%
                                       dplyr::select(date_time, Node_A, Node_B, dispatch_out)
