...
# need to add in any regions with zero import
if(nrow(trans_import_export) < 26){
  trans_import_export = bind_rows(trans_import_export,
                                  data.frame(Node = as.character(setdiff(c(1:26), c(trans_import_export$Node))),
                                             mean_import = 0))
}
...
cap_import_table_data %>% gt(auto_align = 'auto') %>%
                          ...
                          gtsave(filename = paste0('table_',plot_name,'_',year,'.png'),
                                 path = 'figs/')
...
SREX_in_out_plot = 
  ...
  geom_polygon(data = trans_in_out_srex,
               ...
               col = 'transparent',
               size = 0) +
  geom_polygon(data = trans_in_out_srex %>% dplyr::filter(mean_import == 0),
...
ggdraw() +
  ...
  ggsave(paste0('figs/figure_',plot_name,'_',year,'.pdf'),
         device = 'pdf',
         width = 11,
         height = 8.5)
...
trans_table_data %>% gt(auto_align = 'auto') %>%
  ...
  cols_width(vars(Variable) ~ px(400),
             vars(Value) ~ px(100)) %>%
  opt_table_font(font = list(google_font(name = "Spectral"))) %>%
  gtsave(filename = paste0('table_transmission_stats_',plot_name,'_',year,'.png'),
         path = 'figs/')
