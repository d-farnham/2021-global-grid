plot_wind_solar_capacity_import = function(year = 2018,
                                           ...
                                           hour_b = 13.5,
                                           hour_c = 22.5,
                                           date_a = as.Date('2018-12-21'),
                                           date_b = as.Date('2018-06-21')){

# extract the transmission results
source('R/extract-transmission-dispatch-outputs.R')
trans_outputs = 
    extract_trans_dispatch_outputs(year,
                                   gg_output_file)
...
trans_dispatch = trans_outputs %>% dplyr::mutate(hour = lubridate::hour(date_time) + lubridate::minute(date_time)/60,
                                  ...
                                  dplyr::mutate(hour_id = ifelse(hour == hour_a, hour_a,
                                                ...
                                                week_id = ifelse((date_a - 4) < date & date < (date_a + 4), as.character(date_a), 
                                                                 ifelse((date_b - 4) < date & date < (date_b + 4), as.character(date_b), NA))) %>%
                                  dplyr::filter(!is.na(hour_id) &
                                                !is.na(week_id)) %>%
                                  dplyr::mutate(hour_week_id = paste0(hour_id, ';', as.character(week_id),' UTC'),
                                                hour_week_label = hour_week_id) %>%
                                                # hour_week_label = as.POSIXct(paste0(week_id,' ',hour_id,':00 UTC'), tz = 'UTC'),
                                                # hour_week_label = paste0(format(hour_week_label,'%H:%M UTC'),' for 7 days around ', format(hour_week_label,'%B %d, %Y'))) %>%
                                  dplyr::group_by(Node_A, Node_B, hour_week_label) %>%
                                  dplyr::summarise(mean_dispatch = mean(dispatch))


# compute the net import for each region for each of the regions
regional_import = bind_rows(
trans_dispatch %>% dplyr::group_by(Node_A, hour_week_label) %>%
                   dplyr::summarise(import = mean(-mean_dispatch)) %>%
                   dplyr::ungroup() %>%
                   setNames(c('node', 'hour_week_label', 'import')),
trans_dispatch %>% dplyr::group_by(Node_B, hour_week_label) %>%
                   dplyr::summarise(import = mean(mean_dispatch)) %>%
                   dplyr::ungroup() %>%
                   setNames(c('node', 'hour_week_label', 'import'))
) %>% dplyr::group_by(node, hour_week_label) %>%
      dplyr::summarise(import = sum(import))
...
trans_dispatch_long = trans_dispatch %>% merge(wire_IDs,
                                               by = c('Node_A', 'Node_B')) %>%
  reshape2::melt(, id.vars = c('hour_week_label', 'wire_ID', 'mean_dispatch'), value.name = 'node')
...
fill_lims = ceiling(max(abs(SREX_import_plot$import/1e6))/20)*20
...
dispatch_plots[[ttime]] = 
  ...
  geom_polygon(data = SREX_import_plot %>% dplyr::filter(hour_week_label == ttime),
               aes(long, lat, group = piece_LAB, fill = import / 1e6),
               alpha = 1,
               col = 'transparent',
               size = 0) +
  geom_polygon(data = SREX_import_plot %>% dplyr::filter(hour_week_label == ttime &
  ...
  geom_path(data = trans_regions_non_long %>% dplyr::filter(hour_week_label == ttime) %>% 
                                              dplyr::filter(mean_dispatch > 0), # reverse trans -- arrow ends on first node
            aes(x = center_lon, y = center_lat, group = wire_ID, size = mean_dispatch/1e6),
  ...
  geom_segment(data = trans_regions_long_wires %>% dplyr::filter(hour_week_label == ttime) %>% 
                                                   dplyr::filter(mean_dispatch > 0 &
                                                                 variable == 'Node_A'),
               aes(x = center_lon, xend = center_lon_2, y = center_lat, yend = center_lat_2, group = wire_ID, size = mean_dispatch/1e6),
               arrow = arrow(angle = arrow_angle, length = unit(arrow_length, "inches"),
                             ends = "first", type = "closed"),
               alpha = 1) +
  geom_segment(data = trans_regions_long_wires %>% dplyr::filter(hour_week_label == ttime) %>% dplyr::filter(mean_dispatch > 0 &
                                                                        variable == 'Node_B'),
               aes(x = center_lon, xend = center_lon_2, y = center_lat, yend = center_lat_2, group = wire_ID, size = mean_dispatch/1e6),
               arrow = arrow(angle = arrow_angle, length = unit(arrow_length, "inches"),
                             ends = "last", type = "closed"),
...
leg <- get_legend(
  ggplot() + 
    ...
    geom_path(data = trans_regions_non_long, # reverse trans -- arrow ends on first node
              aes(x = center_lon, y = center_lat, group = wire_ID, size = mean_dispatch/1e6),
              arrow = arrow(angle = arrow_angle, length = unit(0.125, "inches"),
                            ends = "last", type = "closed"),
              alpha = 1) +
    scale_size_continuous(name = 'Transmission \n cap. (GW)',
                          breaks = c(1,25,100,250),
...
sun_winter_long = melt(sun_winter, id.vars = c('lat', 'lon', 'dateTime', 'sunrise', 'next_sunrise', 'sunset', 'prior_sunset'),
                       variable.name = 'hour',
...
night_pol_temp = sun_winter_long %>% dplyr::filter(hour == hhour &
                                              light_or_dark == 'dark') %>%
                            dplyr::select(lat, lon, sunrise, next_sunrise, sunset, prior_sunset) %>%
                            dplyr::mutate(dateTime = as.POSIXct(paste0(date_a), tz = 'UTC') + 3600 * hhour) %>%
                            dplyr::mutate(sunrise_dif = sunrise - dateTime,
                                          next_sunrise_dif = next_sunrise - dateTime,
                                          sunset_dif = sunset - dateTime,
                                          prior_sunset_dif = prior_sunset - dateTime) %>%
  dplyr::select(lat, lon, prior_sunset_dif) %>%
  melt(id.vars = c('lat','lon')) %>%
  dplyr::group_by(lon) %>%
  dplyr::mutate(min_value = min(abs(value), na.rm = TRUE)) %>%
  ungroup() %>%
  dplyr::filter(abs(value) == min_value) %>%
  dplyr::arrange(lon) %>%
  dplyr::mutate(hour = hhour)

winter_day_night_plots[[pplot_num]] =
  ggplot(night_pol_temp) +
  borders("world", colour = "transparent", fill = "gray") +
  geom_ribbon(aes(lon, ymin = lat), ymax = 90, alpha = 0.65) +
  theme_classic() +
  theme(axis.text = element_blank(),
        axis.ticks = element_blank(),
        axis.title = element_blank(),
...
for(hhour in c(hour_a, hour_b, hour_c)){
  ...
  sun_summer = sun_summer %>% dplyr::mutate(temp = ifelse(is.na(sunset), 'dark',
                                                          ifelse(time_temp > as.POSIXct(sunrise,tz = 'UTC') &
                                                                   time_temp < as.POSIXct(sunset,tz = 'UTC') |
                                                                   time_temp > as.POSIXct(next_sunrise,tz = 'UTC') |
                                                                   time_temp < as.POSIXct(prior_sunset,tz = 'UTC'),
...
sun_summer_long = melt(sun_summer, id.vars = c('lat', 'lon', 'dateTime', 'sunrise', 'next_sunrise', 'sunset', 'prior_sunset'),
                       variable.name = 'hour',
                       value.name = 'light_or_dark')
...
for(pplot_num in 1:3){
  ...
  hhour = c(hour_a, hour_b, hour_c)[pplot_num]
  day_pol_temp = sun_summer_long %>% dplyr::filter(hour == hhour &
                                                       light_or_dark == 'dark') %>%
    dplyr::select(lat, lon, sunrise, next_sunrise, sunset, prior_sunset) %>%
    dplyr::mutate(dateTime = as.POSIXct(paste0(date_b, hhour, ':00:00', tz = 'UTC'))) %>%
    dplyr::mutate(sunrise_dif = sunrise - dateTime,
                  next_sunrise_dif = next_sunrise - dateTime,
                  sunset_dif = sunset - dateTime,
                  prior_sunset_dif = prior_sunset - dateTime) %>%
  ...
  summer_day_night_plots[[pplot_num]] =
    ggplot(day_pol_temp) +
    borders("world", colour = "transparent", fill = "gray") +
    geom_ribbon(aes(lon, ymax = lat), ymin = -90, alpha = 0.65) +
    theme_classic() +
    theme(axis.text = element_blank(),
          axis.ticks = element_blank(),
          axis.title = element_blank(),
...
day_night_plot_size = 0.1
day_night_plot_v = 0.0
day_night_plot_h = 0.225


ggdraw() +
  draw_plot(dispatch_plots[[5]], x = 0, y = (2*0.315), width = 0.5, height = 0.315, scale = 1.15) +
  draw_plot(dispatch_plots[[1]], x = 0, y = 0.315, width = 0.5, height = 0.315, scale = 1.15) +
  draw_plot(dispatch_plots[[3]], x = 0, y = 0, width = 0.5, height = 0.315, scale = 1.15) +
  draw_plot(dispatch_plots[[6]], x = 0.5, y = (2*0.315), width = 0.5, height = 0.315, scale = 1.15) +
  draw_plot(dispatch_plots[[2]], x = 0.5, y = 0.315, width = 0.5, height = 0.315, scale = 1.15) +
  draw_plot(dispatch_plots[[4]], x = 0.5, y = 0, width = 0.5, height = 0.315, scale = 1.15) +
  draw_plot(winter_day_night_plots[[1]], x = 0.5 + day_night_plot_h, y = (2*0.315) + day_night_plot_v, width = day_night_plot_size, height = day_night_plot_size, scale = 1.25) +
  draw_plot(winter_day_night_plots[[2]], x = 0.5 + day_night_plot_h, y = 0.315 + day_night_plot_v, width = day_night_plot_size, height = day_night_plot_size, scale = 1.25) +
  draw_plot(winter_day_night_plots[[3]], x = 0.5 + day_night_plot_h, y = 0 + day_night_plot_v, width = day_night_plot_size, height = day_night_plot_size, scale = 1.25) +
  draw_plot(summer_day_night_plots[[1]], x = 0 + day_night_plot_h, y = (2*0.315) + day_night_plot_v, width = day_night_plot_size, height = day_night_plot_size, scale = 1.25) +
  draw_plot(summer_day_night_plots[[2]], x = 0 + day_night_plot_h, y = 0.315 + day_night_plot_v, width = day_night_plot_size, height = day_night_plot_size, scale = 1.25) +
  draw_plot(summer_day_night_plots[[3]], x = 0 + day_night_plot_h, y = 0 + day_night_plot_v, width = day_night_plot_size, height = day_night_plot_size, scale = 1.25) +
  draw_plot(leg, x = 0.205, y = 0.425, width = 0.6, height = 0.4, scale = 1) +
